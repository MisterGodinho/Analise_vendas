...
@st.cache_data(show_spinner="Otimizando 29MB...")
    ...
    if len(df_final) > 0:
        ...
        if len(anos_unicos) > 1:
            ...
            with col_l1:
                ...
                st.dataframe(dfl1.style.format({'valor':'R$ {:,.0f}', '% Total':'{:.1f}%'}), use_container_width=True, hide_index=True)
            with col_l2:
                st.write(f"**{ano0}**")
                ...
                st.dataframe(dfl0.style.format({'valor':'R$ {:,.0f}', '% Total':'{:.1f}%'}), use_container_width=True, hide_index=True)

            st.divider()
            st.subheader("Analise Inteligente")
            fat1 = df_final[df_final['ano']==ano1]['valor'].sum()
            fat0 = df_final[df_final['ano']==ano0]['valor'].sum()
            cresc = ((fat1 / fat0) - 1) * 100 if fat0 > 0 else 0
            st.metric(f"Crescimento {ano0} vs {ano1}", f"{cresc:.2f}%", delta=f"R$ {fat1-fat0:,.0f}")
        else:
            st.info("Selecione 2 anos para ver Ranking e Crescimento")
    else:
        st.warning("Nenhum dado com os filtros. Tente marcar mais meses/lojas.")
else:
