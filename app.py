...
if uploaded_files and len(uploaded_files) >= 2:
    ...
    if len(df) > 0:
        ...
        if len(df['ano'].unique()) > 1:
            ...
            x1,x2,x3 = st.columns(2) # CORRIGIDO
            ...
            col_p1, col_p2 = st.columns(2) # CORRIGIDO
            with col_p1:
                st.write(f"**{ano1}**")
            ...
            col_l1, col_l2 = st.columns(2) # CORRIGIDO AQUI - ERA O ERRO DA FOTO
            with col_l1:
                st.write(f"**{ano1}**")
                df_temp1 = df[df['ano']==ano1]
            ...
            with tab1:
                ...
                st.dataframe(df_rank_prod[['Posição','produto','valor','% do Total']].style.format({'valor':'R$ {:,.
