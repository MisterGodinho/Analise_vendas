            ...
            for nome_arquivo in z.namelist():
                ...
                with z.open(nome_arquivo) as f:
                    if nome_arquivo.endswith('.xlsx'):
                        ...
                        df_temp = pd.read_csv(f, sep=';', usecols=[5,6,8,9,16], names=['loja','data','produto','categoria','valor_total'], header=0, encoding='latin-1', on_bad_lines='skip')
                    else: 
                        continue
...
if uploaded_files:
    if len(uploaded_files) < 2: 
        st.warning("⚠️ Faz upload dos 2 arquivos")
    else:
        df = carregar_dados(uploaded_files)
        
        df['valor_total'] = df['valor_total'].astype(str).str.replace('.', '').str.replace(',', '.')
        ...
        categorias = st.sidebar.multiselect("Categoria", sorted(df_temp['categoria'].unique()), sorted(df_temp['categoria'].unique())) # CORRIGIDO ) AQUI
        ...
        if len(df) > 0:
            ...
            col1.metric("Faturamento", "R$ {:,.2f}".format(faturamento_total))
