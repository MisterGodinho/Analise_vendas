...
if uploaded_file:
    try:
        # FORÇA LER A PRIMEIRA ABA E PULAR LINHAS VAZIAS
        ...
        # Só renomeia as colunas que existem
        df = df.rename(columns={k:v for k,v in mapa.items() if k in df.columns})

        # Verifica se as colunas essenciais existem
        colunas_necessarias = ['data', 'loja', 'categoria', 'valor_total']
        for col in colunas_necessarias:
            if col not in df.columns:
                st.error(f"Erro: Coluna '{col}' não encontrada. Verifique o nome no Excel.")
                st.stop()
        ...
        st.error(f"Deu erro ao ler o arquivo: {e}") # AGORA VAI MOSTRAR O ERRO
